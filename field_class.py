...
class Field:
    ...
    def grow(self,light,food,water):
        #grow the crops (light and water are availabale to all crops in same
        #amount
        if len(self._crops) > 0:
            for crop in self._crops:
                crop.grow(light,water)
        if len(self._animals) > 0:
            #grow the animals (water available to all animals, same amount
            #but food must be shared
            food_required = 0
            #get a total of food required
            for animal in self._animals:
                needs = animal.needs()
                food_required += needs["food need"]
            #if we have more food available than required, work out
            #additional food
            ...
            for animal in self._animals:
                ...
                if food >= needs["food need"]:
                    ...
                    #see if any food left
                    if additional_food > 0:
                        #remove some add food for this animal
                        additional_food -= 1
                        #add to feed to be given to the animal
                        feed += 1
                #grow animal
...
def manual_grow(field):
    #get light water and food from user
    valid = False
    while not valid:
        try:
...
def display_animal_menu():
    print()
    print("Which animal tpye would you like to add?" )
    print()
    print("1. Cow")
    print("2. Sheep")
    print()
    print("0. I dont want to add an animal - return to main menu")
    print()
    print("Please select an option from the menu")
